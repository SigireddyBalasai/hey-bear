...
function generateVoiceResponse(message: string) {
  try {
    console.log(
      `Generating voice response with message: "${message.slice(0, 100)}${message.length > 100 ? '...' : ''}"`
    );
    console.log(
      '[VoiceTranscription]',
      `Generating voice response: "${message.slice(0, 100)}${message.length > 100 ? '...' : ''}"`
    );
...
function sanitizeMessage(message: string): string {
  ...
  try {
    return (
      message
        .replaceAll('&', 'and') // Replace & with 'and' instead of &amp; to avoid TTS issues
        .replaceAll('<', '') // Remove < completely
        .replaceAll('>', '') // Remove > completely
        .replaceAll('"', '') // Remove quotes completely
        .replaceAll("'", '') // Remove apostrophes completely
