...
export async function POST(req: Request) {
  ...
  try {
    ...
    const from =
      url.searchParams.get("from") || (formData.get("From") as string);
    ...
    console.log(
      `Query parameters: ${JSON.stringify(Object.fromEntries(url.searchParams))}`,
    );
    logTwilio(
      "VoiceTranscription",
      "Query parameters",
    ...
    if (!speechResult && callSid) {
      logTwilio(
        "VoiceTranscription",
        "Initial voice call detected - starting greeting flow",
      );
      ...
      if (!assistantId) {
        logTwilioError(
          "VoiceTranscription",
          "Missing No-show parameter for initial voice call",
        );
    ...
    if (speechResult) {
      ...
      try {
        ...
        const systemMessage =
          "You are responding to a voice call. Keep your response concise, conversational, and under 150 words. Avoid using special characters or symbols that might be difficult to pronounce.";

        console.log(`Calling assistant chat API at: ${apiUrl}`);
        console.log(
          `Request payload for voice: ${JSON.stringify({
        ...
        try {
          ...
          console.log(
            `Chat API response data: ${JSON.stringify(responseData)}`,
          );
        } catch (parseError) {
          console.error(
            "Failed to parse chat API response as JSON:",
            parseError,
          );
          return generateVoiceResponse(
        ...
        const aiResponse =
          responseData.response || "I'm sorry, I couldn't generate a response.";
        console.log(
          `AI response for voice: "${aiResponse.substring(0, 100)}${aiResponse.length > 100 ? "..." : ""}"`,
        );
      ...
      if (speechError) {
        ...
        logTwilioError(
          "VoiceTranscription",
          `Speech error reported by Twilio: ${speechError}`,
        );
      }
...
function generateVoiceResponseWithSimpleGather(message: string) {
  try {
    ...
    logTwilio(
      "VoiceTranscription",
      `Generating voice response with follow-up gather option`,
    );
...
<Response>
  ...
  <Say voice="alice">Thank you for calling. Goodbye.</Say>
    ...
    logTwilioError(
      "VoiceTranscription",
      "Error generating voice response with gather",
      error,
    );
...
function generateVoiceResponse(message: string) {
  try {
    console.log(
      `Generating voice response with message: "${message.substring(0, 100)}${message.length > 100 ? "..." : ""}"`,
    );
    logTwilio(
      "VoiceTranscription",
      `Generating voice response: "${message.substring(0, 100)}${message.length > 100 ? "..." : ""}"`,
    );
