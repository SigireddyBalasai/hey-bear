...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (body) {
      console.log(
        `Message content: "${body.substring(0, 100)}${body.length > 100 ? "..." : ""}"`,
      );
    ...
    if (
      process.env.VERIFY_WEBHOOK_TOKEN === "true" &&
    ...
    try {
      ...
      logTwilio(
        "Webhook",
        `Calling chat API with message: ${body.substring(0, 30)}${body.length > 30 ? "..." : ""}`,
      );
      ...
      if (!chatResponse.ok) {
        ...
        logTwilioError("Webhook", `Chat API error: ${chatResponse.status}`, {
          errorText,
        });
      ...
      console.log(
        `AI response: "${aiResponse.substring(0, 100)}${aiResponse.length > 100 ? "..." : ""}"`,
      );
      logTwilio(
        "Webhook",
        `AI generated SMS response: ${aiResponse.substring(0, 50)}${aiResponse.length > 50 ? "..." : ""}`,
      );
      ...
      if (insertError) {
        ...
        logTwilioError(
          "Webhook",
          "Failed to save interaction to database",
          insertError,
        );
      } else {
