...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Get assistant count
    const { count: totalAssistants, error: assistantCountError } =
      await supabase
        .from("assistants")
        .select("id", { count: "exact", head: true });
    ...
    const { data: recentInteractions, error: recentInteractionsError } =
      await supabase
        .from("interactions")
    ...
    const totalTokens =
      ...
      0;
    const totalCost =
      recentInteractions?.reduce((sum, i) => sum + (i.cost_estimate || 0), 0) ||
      0;
    const errorCount =
      recentInteractions?.filter((i) => i.is_error).length || 0;
