...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      const tokens =
        hourData?.reduce((sum, i) => sum + (i.token_usage || 0), 0) || 0;
    ...
    const lastHourErrors =
      lastHourData?.filter((item) => item.is_error).length || 0;
    const errorRate =
      lastHourRequests > 0 ? lastHourErrors / lastHourRequests : 0;
    ...
    return NextResponse.json({
      summary: {
        ...
        tokens: {
          hour:
            lastHourData?.reduce((sum, i) => sum + (i.token_usage || 0), 0) ||
            0,
          day:
            lastDayData?.reduce((sum, i) => sum + (i.token_usage || 0), 0) || 0,
          week:
            lastWeekData?.reduce((sum, i) => sum + (i.token_usage || 0), 0) ||
            0,
        },
        costs: {
          hour:
            lastHourData?.reduce((sum, i) => sum + (i.cost_estimate || 0), 0) ||
            0,
          day:
            lastDayData?.reduce((sum, i) => sum + (i.cost_estimate || 0), 0) ||
            0,
          week:
            lastWeekData?.reduce((sum, i) => sum + (i.cost_estimate || 0), 0) ||
            0,
        },
        errors: {
