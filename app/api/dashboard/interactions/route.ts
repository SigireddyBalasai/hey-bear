...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const startDate =
      url.searchParams.get("startDate") || getDefaultStartDate();
    ...
    let query = supabase
      .from("interactions")
      .select(
        "id, interaction_time, assistant_id, user_id, request, response, duration, chat",
      )
      .eq("user_id", userId)
    ...
    query = query
      .gte("interaction_time", startDateFormatted)
      .lte("interaction_time", endDateFormatted);

    if (searchTerm) {
      console.log("Applying search term filter:", searchTerm);
      query = query.or(
        `request.ilike.%${searchTerm}%,response.ilike.%${searchTerm}%`,
      );
    }
    ...
    const { data: chatData, error: chatError } = await query.range(
      (page - 1) * pageSize,
      page * pageSize - 1,
    );
    ...
    const uniqueAssistantIds = new Set(
      interactions.map((i) => i.assistant_id).filter(Boolean),
    ).size;
    ...
    const stats: DashboardStats = {
      ...
      interactionsPerContact:
        uniqueAssistantIds > 0
          ? Math.round((totalCount / uniqueAssistantIds) * 10) / 10
          : 0,
