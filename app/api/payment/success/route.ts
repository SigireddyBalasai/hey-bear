...
import type { AssistantConfigData, WebhookPayload } from '@/types/api.types';
...
import { fetchPaymentSession, updatePaymentSession } from '@/utils/payment/session-management';
import {
...
async function processPaymentSuccess(
  ...
  try {
    ...
    try {
      const { authUserId: parsedUserId, internalPaymentSessionId: parsedSessionId } =
    ...
    if (session.subscription) {
      try {
        ...
        console.log('[PAYMENT SUCCESS] Fetching subscription details from Stripe...');
        ...
        if (subscription.items.data.length > 0) {
          ...
          console.log('[PAYMENT SUCCESS] Product ID from subscription:', productId);
          ...
          } else if (productId === SUBSCRIPTION_PLANS.BUSINESS.stripeProductId) {
            internalPlanId = 'business';
            console.log('[PAYMENT SUCCESS] Matched BUSINESS plan');
          } else {
            console.error(`[PAYMENT SUCCESS] Unknown Stripe product ID: ${productId}`);
    ...
    try {
      ...
      if (session.subscription && result.assistantId) {
        try {
          ...
          if (updateError) {
            console.error('[PAYMENT SUCCESS] Failed to update subscription with Stripe ID:', {
              error: updateError.message,
              assistantId: result.assistantId,
              subscriptionId: session.subscription,
            });
          } else {
            console.log('[PAYMENT SUCCESS] Successfully updated subscription with Stripe ID:', {
              assistantId: result.assistantId,
              subscriptionId: session.subscription,
            });
          }
        } catch (updateSubError) {
          console.error('[PAYMENT SUCCESS] Error updating subscription with Stripe ID:', {
            error:
              updateSubError instanceof Error ? updateSubError.message : String(updateSubError),
            assistantId: result.assistantId,
            subscriptionId: session.subscription,
          });
        }
