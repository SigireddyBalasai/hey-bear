...
const logger = {
  info: (message: string, data?: any) => {
    console.log(
      `[CRAWL INFO] ${new Date().toISOString()} - ${message}`,
      data ? data : "",
    );
  },
  error: (message: string, error?: any) => {
    console.error(
      `[CRAWL ERROR] ${new Date().toISOString()} - ${message}`,
      error ? error : "",
    );
  },
  warn: (message: string, data?: any) => {
    console.warn(
      `[CRAWL WARNING] ${new Date().toISOString()} - ${message}`,
      data ? data : "",
    );
  },
  debug: (message: string, data?: any) => {
    console.debug(
      `[CRAWL DEBUG] ${new Date().toISOString()} - ${message}`,
      data ? data : "",
    );
  },
  http: (
    ...
    console.log(
      `[CRAWL HTTP ${direction}] ${timestamp} - ${method} ${url} ${statusText}`,
      details ? details : "",
    );
  },
};
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (assistantError) {
      logger.error(
        `Error fetching assistant with ID ${assistantId}`,
        assistantError,
      );
      return NextResponse.json(
        {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "Firecrawl API key is missing. Please check your environment variables.",
        },
        { status: 500 },
      );
    }
    ...
    if (!crawlResponse.ok) {
      ...
      logger.error(
        `Crawl request failed with status ${errorStatus}`,
        errorData,
      );
      ...
      return NextResponse.json(
        {
          ...
          message:
            errorData?.detail || `HTTP error! status: ${crawlResponse.status}`,
    ...
    while (
      ...
      pollCount++;
      logger.debug(
        `Polling task status (attempt ${pollCount}/${maxPolls}): ${task_id}`,
      );
        ...
        return NextResponse.json(
          {
            ...
            message:
              errorData?.detail || `HTTP error! status: ${taskResponse.status}`,
          },
          { status: 500 },
        );
      }
      ...
      return NextResponse.json(
        {
          status: "pending",
          message:
            "The crawl task is still in progress. Please check status later.",
          taskId: task_id,
    ...
    if (taskResult?.result) {
      ...
      logger.info(
        `Found ${taskResult.results.length} results, using first result`,
      );
    }
      ...
      return NextResponse.json(
        {
          error: "Failed to extract content",
          message:
            "The crawl completed, but no markdown or HTML content was found.",
          url: resultData.url,
        },
        { status: 400 },
      );
    ...
    try {
      ...
      logger.info(
        `Successfully uploaded URL content to Pinecone with ID: ${uploadResult.id}`,
      );
