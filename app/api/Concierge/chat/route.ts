...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(
      `Request parameters: assistantId=${assistantId}, message length=${message?.length || 0}`,
    );
    console.log(
      `Request message: "${message?.substring(0, 100)}${message?.length > 100 ? "..." : ""}"`,
    );
    ...
    if (!assistantId || !message) {
      console.error(
        `Missing required fields: assistantId=${!!assistantId}, message=${!!message}`,
      );
      return NextResponse.json(
    ...
    if (!subscriptionCheck.isActive) {
      console.error(
        `Subscription check failed: ${subscriptionCheck.error}`,
        subscriptionCheck,
      );
      return NextResponse.json(
        {
    ...
    console.log(
      `Found No-Show: name=${assistantName}, pinecone_name=${pinecone_name}`,
    );
    ...
    try {
      ...
      console.log(
        `No-Show response: "${response.message?.content?.substring(0, 100)}${(response.message?.content?.length || 0) > 100 ? "..." : ""}"`,
      );
      ...
      console.log(
        `Token usage: ${tokenCount} tokens (prompt: ${response.usage?.promptTokens || "N/A"}, completion: ${response.usage?.completionTokens || "N/A"})`,
      );
      ...
      console.log(
        `[${responseTimestamp.toISOString()}] No-Show chat API completed successfully`,
      );
      return NextResponse.json({
        response: response.message?.content || "",
        tokens: tokenCount,
