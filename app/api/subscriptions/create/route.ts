...
export async function POST(req: NextRequest) {
  try {
    ...
    if (pendingError || !pendingAssistant) {
      ...
      return NextResponse.json(
        { error: "Pending assistant not found" },
        { status: 404 },
      );
    }
    ...
    if (pendingAssistant.user_id !== userData.id) {
      return NextResponse.json(
        { error: "Unauthorized: You do not own this assistant" },
        { status: 403 },
      );
    }
    ...
    const priceInCents = planDetails
      ? Math.round(planDetails.price * 100)
      : 1000;
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        params: {
          ...(typeof pendingAssistant.params === "object" &&
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: "Failed to update pending assistant with checkout data" },
        { status: 500 },
      );
    }
