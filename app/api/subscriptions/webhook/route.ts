...
async function handleCheckoutSessionCompleted(session: any, supabase: any) {
  ...
  if (session && session.metadata?.pendingAssistantId) {
    ...
    console.log(
      `Found pending assistant metadata: pendingId=${pendingAssistantId}`,
    );
    ...
    return handleCheckoutSessionForAssistant(
      session,
      supabase,
      pendingAssistant,
    );
    ...
    console.log(
      "Session does not contain pending assistant metadata:",
      session.id,
    );
  }
}
...
async function getPlanIdFromProductId(
  productId: string,
  supabase: any,
): Promise<string | null> {
...
async function handleCheckoutSessionForAssistant(
  session: any,
  supabase: any,
  pendingAssistant: any,
) {
  ...
  if (!planId) {
    console.error(
      `Could not find plan UUID for Stripe product ID: ${stripeProductId}`,
    );
  ...
  try {
    ...
    try {
      ...
      console.log(
        "Attempting to create assistant in Pinecone with name:",
        pinecone_name,
      );
      ...
      const pineconeResult = await pinecone.createAssistant({
        name: pinecone_name,
        instructions:
          pendingAssistant.params?.systemPrompt ||
          `You are a helpful AI assistant named ${pendingAssistant.name}.`,
        metadata: {
          owner: pendingAssistant.user_id,
          description:
            pendingAssistant.params?.description || "No description provided",
      ...
      try {
        ...
        console.log(
          "Created assistant in Pinecone with fallback name:",
          pineconeResult,
        );
    ...
    const currentPeriodEndISO = safeTimestampToISO(
    ...
    console.log(
      `Successfully created new assistant ${newAssistantId} from pending assistant ${pendingAssistant.id}`,
    );
    ...
    console.log(
      `Successfully processed pending assistant ${pendingAssistant.id}`,
    );
  } catch (error) {
...
async function handleSubscriptionCreated(subscription: any, supabase: any) {
  ...
  if (subscription && subscription.metadata?.assistantId) {
    ...
    if (isUpgrade && previousSubscriptionId) {
      try {
        console.log(
          `This is a plan upgrade. Canceling previous subscription ${previousSubscriptionId}`,
        );
        await stripe?.subscriptions.cancel(previousSubscriptionId, {
          prorate: false, // No refunds for upgrades
        });
        console.log(
          `Successfully canceled previous subscription ${previousSubscriptionId}`,
        );
      } catch (cancelError) {
        console.error(
          "Error canceling previous subscription during upgrade:",
          cancelError,
        );
      ...
      .update({
        params: {
          ...
          subscription: {
            ...
            plan: isUpgrade
              ? "business"
              : assistantData.params?.subscription?.plan || "personal",
            currentPeriodEnd: new Date(
              (subscription as any).current_period_end * 1000,
            ).toISOString(),
            createdAt: new Date().toISOString(),
  ...
  if (subscription && subscription.metadata?.pendingAssistantId) {
    ...
    try {
      ...
      if (existingAssistant) {
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            params: {
              ...pendingAssistant.params,
              description:
                pendingAssistant.params?.description ||
        ...
        console.log(
          `Successfully updated assistant ${assistantId} with Pinecone information`,
        );
      } else {
        ...
        if (assistantError) {
          console.error(
            "Error creating assistant in database:",
            assistantError,
          );
          return;
        }
        ...
        console.log(
          `Successfully created new assistant ${newAssistantId} from pending assistant ${pendingAssistantId}`,
        );
      }
      ...
      console.log(
        `Successfully processed pending assistant ${pendingAssistantId} via subscription creation`,
      );
    } catch (error) {
      console.error("Error creating assistant from pending assistant:", error);
    }
