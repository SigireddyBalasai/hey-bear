...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!user) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Assignment - Unauthorized request`,
      );
    ...
    console.log(
      `[${new Date().toISOString()}] Phone Number Assignment - Request body:`,
      {
        assistantId,
        phoneNumber,
    ...
    if (!assistantId || !countryCode) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Assignment - Missing fields`,
      );
      return NextResponse.json(
        {
          error:
            "Missing required fields: assistantId and countryCode are required",
        },
        { status: 400 },
      );
    }
    ...
    if (!phoneNumber) {
      ...
      try {
        ...
        if (!availableNumbers || availableNumbers.length === 0) {
          ...
          try {
            ...
            if (areaCode && areaCode.trim() !== "") {
              Object.assign(purchaseSearchParams, {
                areaCode: parseInt(areaCode.trim(), 10),
              });
            }
            ...
            if (
              !availableNumbersForPurchase ||
              availableNumbersForPurchase.length === 0
            ) {
              const errorMessage =
            ...
            const phoneNumberToPurchase =
              availableNumbersForPurchase[0].phoneNumber;
            ...
            if (!purchasedNumber) {
              console.log(
                `[${new Date().toISOString()}] Phone Number Assignment - Failed to purchase number in ${countryCode}`,
              );
              return NextResponse.json(
                { error: `Failed to purchase number in ${countryCode}` },
                { status: 500 },
              );
            }
            ...
            try {
              ...
              return NextResponse.json(
                {
                  error: `Error adding purchased number to pool: ${poolError.message}`,
                },
                { status: 500 },
              );
            }
            ...
            return NextResponse.json(
              { error: `Error purchasing number: ${purchaseError.message}` },
              { status: 500 },
            );
          }
        } else {
          ...
          if (checkError || !existingNumber) {
            ...
            try {
              ...
              return NextResponse.json(
                {
                  error: `Failed to add number to database: ${dbError.message}`,
                },
                { status: 500 },
              );
            }
        ...
        return NextResponse.json(
          {
            error: `Error searching for phone numbers: ${searchError.message}`,
          },
          { status: 500 },
        );
      }
    ...
    if (!finalWebhookUrl) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Assignment - Missing webhook URL`,
      );
      return NextResponse.json(
    ...
    if (!isValidURL(finalWebhookUrl)) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Assignment - Invalid webhook URL`,
      );
      return NextResponse.json(
    ...
    if (assistantError || !assistantData) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Assignment - No-Show not found`,
      );
    ...
    try {
      ...
      if (!twilioResult || !twilioResult.sid) {
        throw new Error(
          "Failed to create Twilio TwiML app - no app SID returned",
        );
      }
      ...
      if (
        ...
        try {
          ...
          return NextResponse.json(
            {
              error: `Error adding new phone number to database: ${insertError.message || "Unknown error"}`,
            },
            { status: 500 },
          );
        }
      ...
      if (twimlAppSid) {
        const phoneVerification = await verifyTwilioPhoneConfig(
          phoneNumberToAssign,
          twimlAppSid,
        );
        ...
        throw new Error(
          "TwiML app was not created successfully (no app SID available)",
        );
      }
        ...
        twilioDetails: twimlAppDetails
          ? {
              twimlApp: {
                ...
                dateCreated:
                  twimlAppDetails.dateCreated || new Date().toISOString(),
              },
              phoneNumber: {
                number: phoneNumberToAssign,
    ...
    console.error(
      `[${new Date().toISOString()}] Phone Number Assignment - ERROR:`,
      error,
    );
    return NextResponse.json(
...
async function verifyTwilioPhoneConfig(
  ...
  try {
    ...
    if (!incomingPhoneNumbers || incomingPhoneNumbers.length === 0) {
      console.log(
        `[TWILIO][${new Date().toISOString()}] No phone number found for verification`,
      );
      return {
        success: false,
        error: `No Twilio number found matching ${phoneNumber}`,
      };
    }
    ...
    console.log(
      `[TWILIO][${new Date().toISOString()}] Phone verification details:`,
      {
    ...
    if (phoneDetails.smsApplicationSid !== expectedAppSid) {
      console.log(
        `[TWILIO][${new Date().toISOString()}] Phone number has incorrect TwiML app SID`,
      );
      console.log(
    ...
    console.log(
      `[TWILIO][${new Date().toISOString()}] Phone configuration verified successfully`,
    );
    return { success: true };
  } catch (error: any) {
    console.error(
...
async function createAndConfigureTwilioTwiMLApp(
  ...
  try {
    logTwilio(
      "Assignment",
      `Creating TwiML app for phone number: ${phoneNumber}`,
    );
    ...
    if (!incomingPhoneNumbers || incomingPhoneNumbers.length === 0) {
      logTwilioError(
        "Assignment",
        `No phone number found matching ${formattedNumber}`,
      );
      logTwilio(
        "Assignment",
        `Will attempt to purchase a number in ${countryCode}`,
      );
      ...
      try {
        ...
        try {
          ...
          logTwilioError(
            "Assignment",
            `Error purchasing specific number: ${purchaseError.message}`,
          );
          logTwilio(
            "Assignment",
        ...
        throw new Error(
          `Could not find or purchase phone number ${formattedNumber}: ${error.message}`,
        );
      }
    } else {
    ...
    const voiceTranscriptionUrl = new URL(
      "/api/twilio/voice-transcription",
      webhookUrlObj.origin,
    );
    ...
    if (!newTwiMLApp || !newTwiMLApp.sid) {
      logTwilioError(
        "Assignment",
        `Failed to create TwiML app, no SID returned`,
      );
      throw new Error(
        "Failed to create TwiML app: No SID returned from Twilio",
      );
    }
    ...
    if (
      !updatedPhoneNumber ||
      updatedPhoneNumber.smsApplicationSid !== newTwiMLApp.sid
    ) {
      logTwilioError(
        "Assignment",
        `Failed to update phone number with TwiML app SID`,
      );
...
async function cleanupTwilioTwiMLApp(
  phoneNumber: string,
  twimlAppSid: string,
): Promise<void> {
  const accountSid = process.env.TWILIO_ACCOUNT_SID;
  ...
  try {
    ...
    if (incomingPhoneNumbers && incomingPhoneNumbers.length > 0) {
      ...
      console.log(
        `[TWILIO][${new Date().toISOString()}] Clearing TwiML app SID from phone number`,
      );
      ...
      console.log(
        `[TWILIO][${new Date().toISOString()}] Updated phone details:`,
        {
