...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!assistantId || !phoneNumber) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Unassignment - Missing required fields`
      );
    ...
    const { data: _phoneNumberData, error: phoneNumberError } = await supabase
      .from('phone_numbers')
      .select('*')
  ...
  try {
    ...
    if (twimlAppSid) {
      ...
      try {
        ...
        if (error && typeof error === 'object' && 'code' in error && error.code === 20_404) {
