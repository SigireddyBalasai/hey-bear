...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!user) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Unassignment - Unauthorized request`,
      );
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
    console.log(
      `[${new Date().toISOString()}] Phone Number Unassignment - User authenticated: ${user.id}`,
    );

    // Parse the request body
    const { assistantId, phoneNumber } = await request.json();
    console.log(
      `[${new Date().toISOString()}] Phone Number Unassignment - Request body:`,
    ...
    if (!assistantId || !phoneNumber) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Unassignment - Missing fields`,
      );
    ...
    if (assistantError || !assistantData) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Unassignment - Phone number is not assigned to this assistant`,
      );
      return NextResponse.json(
    ...
    if (phoneNumberError || !phoneNumberData) {
      console.log(
        `[${new Date().toISOString()}] Phone Number Unassignment - Phone number not found in database`,
      );
      return NextResponse.json(
    ...
    try {
      ...
      console.log(
        `[${new Date().toISOString()}] Phone Number Unassignment - Clearing Twilio webhook...`,
      );

      // Get the app SID directly from the Twilio API, not from DB
      const cleanupResult = await updateTwilioWebhook(phoneNumber, null);
      console.log(
        `[${new Date().toISOString()}] Phone Number Unassignment - Successfully cleared Twilio webhook`,
        cleanupResult,
      );
    ...
    if (updatePhoneError) {
      console.error(
        `[${new Date().toISOString()}] Phone Number Unassignment - Update phone error:`,
        updatePhoneError,
      );
      return NextResponse.json(
    ...
    if (updateAssistantError) {
      console.error(
        `[${new Date().toISOString()}] Phone Number Unassignment - Update No-Show error:`,
    ...
    console.log(
      `[${endTime.toISOString()}] Phone Number Unassignment - END - Duration: ${duration}ms`,
    );
    ...
    console.error(
      `[${new Date().toISOString()}] Phone Number Unassignment - ERROR:`,
      error,
    );
    return NextResponse.json(
...
async function updateTwilioWebhook(
  ...
  try {
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Starting webhook update for phone number: ${phoneNumber}`,
    );
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Known app SID: ${knownAppSid || "none"}`,
    );
    ...
    const formattedNumber = phoneNumber.startsWith("+")
      ...
      : `+${phoneNumber}`;
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Formatted phone number: ${formattedNumber}`,
    );

    // Find the Twilio phone number
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Looking up phone number in Twilio account...`,
    );
    ...
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Found ${incomingPhoneNumbers.length} matching phone number(s)`,
    );

    if (!incomingPhoneNumbers || incomingPhoneNumbers.length === 0) {
      console.error(
        `[TWILIO UNASSIGN][${new Date().toISOString()}] No phone number found matching ${formattedNumber}`,
      );
    ...
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Phone number details:`,
      {
    ...
    if (!twimlAppSid) {
      console.log(
        `[TWILIO UNASSIGN][${new Date().toISOString()}] No known app SID, fetching phone number details...`,
      );
      ...
      console.log(
        `[TWILIO UNASSIGN][${new Date().toISOString()}] Fetched phone details:`,
        {
    ...
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Found TwiML app SID to remove: ${twimlAppSid || "No SID attached to phone number"}`,
    );
    ...
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Update params for clearing app SIDs:`,
      updateParams,
    );

    // Clear the webhook URL for SMS and Voice
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Updating phone number to clear app SIDs...`,
    );
    ...
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Successfully cleared application SIDs from phone number:`,
      updatedNumber.phoneNumber,
    );
    console.log(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Updated phone details:`,
      {
    ...
    if (twimlAppSid) {
      console.log(
        `[TWILIO UNASSIGN][${new Date().toISOString()}] Attempting to delete TwiML app: ${twimlAppSid}`,
      );
      try {
        await client.applications(twimlAppSid).remove();
        console.log(
          `[TWILIO UNASSIGN][${new Date().toISOString()}] TwiML app deleted successfully: ${twimlAppSid}`,
        );
      } catch (err: any) {
        console.error(
          `[TWILIO UNASSIGN][${new Date().toISOString()}] Failed to delete TwiML app:`,
          err,
        );
        if (err.code === 20404) {
          // App not found is okay - it might have been deleted already
          console.log(
            `[TWILIO UNASSIGN][${new Date().toISOString()}] App ${twimlAppSid} already deleted or not found`,
          );
        } else {
      ...
      console.log(
        `[TWILIO UNASSIGN][${new Date().toISOString()}] No TwiML app SID found to delete`,
      );
    }
    ...
    console.error(
      `[TWILIO UNASSIGN][${new Date().toISOString()}] Error updating Twilio webhook:`,
      error,
    );
    if (error.code) {
      console.error(
        `[TWILIO UNASSIGN][${new Date().toISOString()}] Twilio error code: ${error.code}`,
      );
    }
    if (error.moreInfo) {
      console.error(
        `[TWILIO UNASSIGN][${new Date().toISOString()}] Twilio error info: ${error.moreInfo}`,
      );
    }
    throw error;
  }
