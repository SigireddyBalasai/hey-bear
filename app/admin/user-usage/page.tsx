...
export default function UserUsagePage() {
  ...
  const totalTokens = usageData.reduce(
    (sum, item) => sum + (item.token_usage || 0),
    0,
  );
  ...
  const totalMessages = usageData.reduce(
    (sum, item) => sum + (item.message_count || 0),
    0,
  );
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ~{Math.round(totalMessages / Math.max(1, uniqueUserCount))}{" "}
                  per user
                </p>
              </div>
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Average $
                  {(totalCost / Math.max(1, uniqueUserCount)).toFixed(2)} per
                  user
                </p>
              </div>
