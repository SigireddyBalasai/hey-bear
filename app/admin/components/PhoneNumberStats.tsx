...
export function PhoneNumberStats() {
  ...
  const loadPhoneStats = async () => {
    ...
    try {
      ...
      const formattedPhoneStats = Array.from(phoneData.values()).map(
        (phone) => ({
          ...phone,
      ...
      const activePhones = formattedPhoneStats.filter(
        (p) => p.total_messages > 0,
      ).length;
                  ...
                  <div className="flex justify-center mt-4 gap-6">
                    <div className="text-center">
                      ...
                      <p className="text-lg font-medium">
                        {usageSummary.assigned}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {usageSummary.total
                          ? (
                              (usageSummary.assigned / usageSummary.total) *
                              100
                            ).toFixed(1)
                          : 0}
                        %
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {usageSummary.total
                          ? (
                              (usageSummary.unassigned / usageSummary.total) *
                              100
                            ).toFixed(1)
                          : 0}
                        %
