  ...
  const loadUsageData = async (timeframe: string) => {
    ...
    try {
      ...
      const { totalStats, timeSeriesData, userStats } =
        await fetchUsageData(timeframe);
      ...
      if (timeSeriesData.length > 0) {
        ...
        const currentCost = currentPeriod.reduce(
          (sum, day) => sum + day.costs,
          0,
        );
        const previousCost = previousPeriod.reduce(
          (sum, day) => sum + day.costs,
          0,
        );
        const costChange =
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <div className="text-xs text-muted-foreground">
                ...
                {(
                  (totalStats.errors / Math.max(totalStats.interactions, 1)) *
                  100
                ).toFixed(1)}
                %)
              </div>
                      ...
                      tooltip: {
                        callbacks: {
                          label: function (context) {
                            ...
                            const day = timeSeriesData[dataIndex] || {
                              tokens: 0,
                              interactions: 0,
                            };
                            return [
                    ...
                    plugins: {
                      ...
                      tooltip: {
                        callbacks: {
                          label: function (context) {
                            const dataPoint =
                              tokenDistribution[context.dataIndex];
            ...
            <div className="border rounded-md">
              ...
              <div>
                {userStats.slice(0, 5).map((user, index) => {
                  ...
                  const percentage =
                    totalCost > 0 ? (user.costs / totalCost) * 100 : 0;
