...
export async function fetchUsageData(timeframe: string = "30d") {
  try {
    ...
    const activeUsers = new Set(interactions.map((i) => i.user_id));
    ...
    interactions.forEach((interaction) => {
      ...
      const date = new Date(interaction.interaction_time)
        .toISOString()
        .split("T")[0];
...
export async function fetchSystemMetrics() {
  ...
  try {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const intervalInteractions =
        recentInteractions?.filter((interaction) => {
          ...
          return (
            time >= intervalStart.getTime() && time < intervalEnd.getTime()
          );
        }) || [];

      const tokens = intervalInteractions.reduce(
        (sum, i) => sum + (i.token_usage || 0),
        0,
      );
    ...
    return {
      ...
      totalTokens:
        recentInteractions?.reduce((sum, i) => sum + (i.token_usage || 0), 0) ||
        0,
      errorRate: recentInteractions?.length
        ? recentInteractions.filter((i) => i.is_error).length /
