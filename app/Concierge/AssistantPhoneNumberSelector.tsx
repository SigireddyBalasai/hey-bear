...
export function AssistantPhoneNumberSelector({
  ...
  const fetchAvailablePhoneNumbers = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || "Failed to fetch available phone numbers",
        );
      }
  ...
  const assignPhoneNumber = async () => {
    ...
    setIsAssigning(true);
    console.log(
      `Starting phone number assignment: ${selectedNumber} for assistant: ${assistantId}`,
    );
    ...
    try {
      ...
      if (data.data?.twilioDetails) {
        console.log(
          "Twilio TwiML app details received:",
          JSON.stringify(data.data.twilioDetails, null, 2),
        );

        // Additional check to make sure we have valid TwiML app data
        if (
          data.data.twilioDetails.twimlApp &&
          data.data.twilioDetails.twimlApp.sid
        ) {
  ...
  const unassignPhoneNumber = async () => {
    ...
    try {
      console.log(
        `Unassigning Twilio number: ${currentPhoneNumber} from No-Show : ${assistantId}`,
      );
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || "Failed to unassign Twilio phone number",
        );
      }
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to unassign Twilio phone number",
      );
    } finally {
      setIsAssigning(false);
    }
  ...
  return (
    <div className="space-y-4">
          ...
          <Dialog open={dialogOpen} onOpenChange={handleDialogOpenChange}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Select a phone number to connect to this No-Show This will
                  enable SMS conversations.
                </DialogDescription>
              </DialogHeader>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setUseDefaultWebhook(checked as boolean)
                          }
                        />
                        <Label htmlFor="useDefaultWebhook">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          The webhook URL will handle both SMS messages and
                          voice calls for this No-Show
                        </p>
                      </div>
                    </div>
        ...
        <Card className="bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800">
          ...
          <CardContent className="text-xs space-y-1 text-green-700 dark:text-green-300">
            <p>
              ...
              {twilioAppInfo.twimlApp.name ||
                twilioAppInfo.twimlApp.friendlyName ||
                "SMS Handler"}
            </p>
            <p>
              <strong>App SID:</strong>{" "}
              {twilioAppInfo.twimlApp.sid
                ? `${twilioAppInfo.twimlApp.sid.substring(0, 8)}...`
                : "Not available"}
            </p>
            <p>
              <strong>SMS URL:</strong>{" "}
              {twilioAppInfo.twimlApp.smsUrl || "Not configured"}
            </p>
      ...
      <div className="text-sm text-muted-foreground">
        {currentPhoneNumber ? (
          <p>
            This No-Show can receive and respond to SMS messages via Twilio at
            this number.
          </p>
        ) : (
          <p>
            Assign a Twilio phone number to enable SMS interactions with this
            No-Show{" "}
          </p>
        )}
      </div>
