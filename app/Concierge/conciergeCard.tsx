...
import type { AssistantCardProps } from "@/types/AssistantCardProps";
import { createClient } from "@/utils/supabase/client";

function getDashboardUrl(assistantNameOrId?: string): string {
...
export function AssistantCard({
  ...
  const {
    isLoading: isActionInProgressState,
    setIsLoading: setIsActionInProgressState,
  ...
  const handleToggleStar = async (id: string, isStarred: boolean) => {
    ...
    try {
      ...
      if (error) {
        console.log("Failed to update star status", error);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.log(
          ...
          errorData.error ?? "Failed to delete assistant",
        );

        return;
      }

      console.log("Assistant deleted", `${assistant.name} has been removed`);
    } finally {
      setIsActionInProgressState(false);
    }
  };
  ...
  const handleUpgrade = async (id: string) => {
    ...
    try {
      console.log("Starting upgrade process", `Upgrading ${assistant.name}`);
      ...
      <CardContent className="flex-1">
        <div className="mb-3 flex flex-wrap gap-2">
          ...
          {assistant.plan_name && (
            <Badge
              variant={
                assistant.plan_name === "business" ? "default" : "outline"
              }
        ...
        <div className="flex items-center text-sm text-muted-foreground">
          ...
          <span>
            {assistant.total_messages
              ? `${assistant.total_messages} messages`
              : "No messages yet"}
          </span>
        </div>
