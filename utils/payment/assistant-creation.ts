...
async function createAssistantDirect(params: {
  ...
  const { error: configError } = await supabase.from('assistant_configs').insert(configData);

  if (configError) {
    // Clean up assistant record if config creation fails
    await supabase.from('assistants').delete().eq('id', assistantId);
    throw new Error(`Failed to create assistant config: ${configError.message}`);
  }
  ...
  if (subscriptionError) {
    ...
    await supabase.from('assistants').delete().eq('id', assistantId);
    throw new Error(`Failed to create assistant subscription: ${subscriptionError.message}`);
  }
  ...
  const { error: activityError } = await supabase.from('assistant_activity').insert(activityData);
