...
export function logOutgoingSms(
  ...
  logTwilio(
    "OutgoingSMS",
    `To: ${to}, From: ${from}, Message: ${truncatedMessage}`,
    metadata,
  );
}
 ...
 */
export function logIncomingSms(
  from: string,
  to: string,
  message: string,
  metadata?: any,
  ...
  logTwilio(
    "IncomingSMS",
    `From: ${from}, To: ${to}, Message: ${truncatedMessage}`,
    metadata,
  );
}
 ...
 */
export function checkSmsErrorPatterns(message: string): {
  hasErrors: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (/[^\w\s.,?!;:()\-@#']/.test(message)) {
    errors.push(
      "Contains potential special characters that may affect delivery",
    );
  }
