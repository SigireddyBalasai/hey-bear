...
export function getUsageLimit(usageType: UsageType, subscription: any): number {
  ...
  const plan =
    (subscription as { plan?: string })?.plan?.toLowerCase() === "business"
      ? "BUSINESS"
      : "PERSONAL";
...
export async function trackUsage(
  ...
  try {
    ...
    const shouldResetMonthly =
      currentUsage && isTimeToResetMonthly(currentUsage.last_reset);

    if (
      shouldResetMonthly &&
      (usageType === UsageType.MESSAGE_RECEIVED ||
        usageType === UsageType.MESSAGE_SENT)
    ) {
...
export async function isLimitReached(
  ...
  try {
    ...
    if (usageResponse.error || assistantResponse.error) {
      console.error(
        "Error fetching usage or assistant data:",
    ...
    const subscription =
      typeof assistantParams === "object" &&
      assistantParams !== null &&
      "subscription" in assistantParams
        ? assistantParams.subscription
        : null;
    ...
    if (
      ...
      const messageLimit = getUsageLimit(
...
export async function getAssistantUsageStats(assistantId: string) {
  try {
    ...
    if (usageResponse.error || assistantResponse.error) {
      console.error(
        "Error fetching usage or assistant data for stats:",
        usageResponse.error || assistantResponse.error,
      );
      return null;
    }
    ...
    return {
      messages: {
        ...
        remaining: Math.max(
          0,
          PLAN_LIMITS[plan].MESSAGES_MONTHLY -
            ((usage.messages_received || 0) + (usage.messages_sent || 0)),
        ),
      },
      documents: {
        ...
        remaining: Math.max(
          0,
          PLAN_LIMITS[plan].DOCUMENTS - (usage.documents_count || 0),
        ),
      },
      webpages: {
        ...
        remaining: Math.max(
          0,
          PLAN_LIMITS[plan].WEBPAGES - (usage.webpages_crawled || 0),
        ),
      },
