...
export async function handleCheckoutSessionCompleted(
  ...
  if (sessionId && userId) {
    assistantData = await getAssistantDataFromPaymentSession(
      supabase,
      sessionId,
      userId,
    );
  ...
  if (
    ...
    try {
      ...
      if (assistantData) {
        assistantId = await createAssistantWithConfig(
          supabase,
          userId,
          assistantData,
        );
      ...
      if (assistantId) {
        const success = await createAssistantSubscription(
          supabase,
          assistantId,
          subscriptionId,
        );
...
export async function handleInvoicePaymentSucceeded(
  ...
  const subscriptionRef = (
    invoice as { subscription?: string | { id: string } }
  ).subscription;
