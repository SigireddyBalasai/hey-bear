...
export function useFilters() {
  const { dateRange, assistantId, searchTerm, currentPage, pageSize, setCurrentPage } = useData();
   ...
   */
  const filterByPhoneNumber = (interactions: TransformedInteraction[], phoneNumber?: string) => {
   ...
   */
  const filterByMessage = (interactions: TransformedInteraction[], message?: string) => {
    if (!message) return interactions;
    return interactions.filter(i => i.message.toLowerCase().includes(message.toLowerCase()));
  };

  /**
   * Filter interactions by assistant name
   */
  const filterByAssistant = (interactions: TransformedInteraction[], assistantName?: string) => {
    if (!assistantName) return interactions;
    return interactions.filter(i =>
      i.assistant_name?.toLowerCase().includes(assistantName.toLowerCase())
    );
  };
   ...
   */
  const filterByStatus = (interactions: TransformedInteraction[], status?: string) => {
  ...
  const _applyLegacyInteractionFilters = (
    ...
    if (options.withSorting) {
      query = query.order(appliedFilters.sortBy, {
        ascending: appliedFilters.sortDirection === 'asc',
      });
    }
  ...
  const fetchFilteredInteractions = async (
    ...
    try {
      const query = applyInteractionFilters(filters).select('*, phone_numbers!left(phone_number)', {
        count: 'exact',
      });
  ...
  const fetchFilteredUsageStatistics = async (
    ...
    try {
      const query = applyUsageStatisticsFilters(filters).select('*, assistants!inner(name)');
   ...
   */
  const fetchFilteredPhoneNumbers = async (filters: Partial<Tables<'phone_numbers'>> = {}) => {
    try {
      const query = applyPhoneNumberFilters(filters).select('*, assistants!inner(name)');
