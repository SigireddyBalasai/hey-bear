...
export function RealTimeMonitor({
  ...
  useEffect(() => {
    ...
    updateHealthMetrics(initialData);
  }, []);

  // Extract health metrics calculation to a reusable function
  const updateHealthMetrics = useCallback((data: SystemStats[]) => {
    // Count alerts
    const errors = data.reduce((count, item) => count + item.apiErrors, 0);
    setAlertsCount(prev => prev + errors);
    
    // Update health status
    const avgLatency = data.reduce((sum, item) => sum + item.apiLatency, 0) / data.length;
  ...
  // Generate new random stat
  const generateStat = useCallback((): SystemStats => ({
    timestamp: new Date(),
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // Update health metrics with just the new stat
      if (newStat.apiErrors > 0) {
        setAlertsCount(a => a + newStat.apiErrors);
      }
  ...
  }, [isMonitoring, refreshInterval, generateStat]);

  // Update health status whenever stats change
  useEffect(() => {
    if (stats.length === 0) return;
    
    const avgLatency = stats.reduce((sum, item) => sum + item.apiLatency, 0) / stats.length;
    const errorRate = stats.reduce((sum, item) => sum + item.apiErrors, 0) / stats.length;
  ...
  const latestStats = useMemo(() => stats[stats.length - 1] || {
    apiLatency: 0,
    apiErrors: 0,
    apiRequests: 0,
    cpuUsage: 0,
