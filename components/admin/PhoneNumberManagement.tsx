...
export function PhoneNumberManagement({ initialTab = "assigned" }: PhoneNumberManagementProps) {
  const [assignedNumbers, setAssignedNumbers] = useState<AssignedPhoneNumber[]>([]);
  const [assistants, setAssistants] = useState<Array<{ id: string; name: string; user_id: string; owner?: string; owner_name?: string }>>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setAssignedNumbers(assigned as AssignedPhoneNumber[]);
      setAssistants(unassignedAssistants as Array<{ id: string; name: string; user_id: string; owner?: string; owner_name?: string }>);
    } catch (error) {
                  ...
                  <div className="divide-y">
                    {assignedNumbers.map((phone) => (
                      <div key={phone.id} className="grid grid-cols-3 px-4 py-3 items-center">
                        <div>
                          <div className="font-medium">{phone.assistants?.name || 'Unknown Assistant'}</div>
                    ...
                    <SelectContent>
                      {assistants.map(assistant => (
                        <SelectItem key={assistant.id} value={assistant.id}>
                          {assistant.name} (Owner ID: {assistant.user_id || 'N/A'})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
