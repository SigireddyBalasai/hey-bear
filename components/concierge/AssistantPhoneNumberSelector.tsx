...
export function AssistantPhoneNumberSelector({
  ...
  const [twilioAppInfo, setTwilioAppInfo] = useState<TwilioAppInfo | null>(null);
  ...
  // Handle webhook checkbox change
  const handleWebhookToggle = (checked: boolean) => {
    setUseDefaultWebhook(checked);
    setWebhook(checked ? getDefaultWebhookUrl() : '');
  };
