...
CREATE OR REPLACE FUNCTION "public"."init_assistant_usage_params"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
            BEGIN
                -- Initialize usage tracking in params if not present
                IF NEW.params IS NULL OR NEW.params::text = '{}' THEN
                    NEW.params = '{"usage": {"documents_count": 0, "webpages_crawled": 0}}';
                ELSIF NOT (NEW.params ? 'usage') THEN
                    NEW.params = jsonb_set(NEW.params, '{usage}', '{"documents_count": 0, "webpages_crawled": 0}');
                END IF;
                RETURN NEW;
            END;
            $$;
...
ALTER FUNCTION "public"."init_assistant_usage_params"() OWNER TO "postgres";
...
CREATE INDEX "idx_assistants_pending" ON "public"."assistants" USING "btree" ("pending");
...
CREATE OR REPLACE TRIGGER "assistant_usage_params_trigger" BEFORE INSERT OR UPDATE ON "public"."assistants" FOR EACH ROW EXECUTE FUNCTION "public"."init_assistant_usage_params"();
...
GRANT ALL ON FUNCTION "public"."init_assistant_usage_params"() TO "anon";
GRANT ALL ON FUNCTION "public"."init_assistant_usage_params"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."init_assistant_usage_params"() TO "service_role";
